...
for FILE in $(find $TMPDIR/addon/AnyKernel3/ramdisk -type f 2>/dev/null | sed "s|$TMPDIR/addon/AnyKernel3||" 2>/dev/null); do
  cp_ch $TMPDIR/addon/AnyKernel3$FILE $BOOTDIR$FILE
done
...
if [ -d /system/addon.d ]; then
  ...
  [ "$(ls -A $TMPDIR/addon/AnyKernel3/ramdisk 2>/dev/null)" ] && cp_ch -rn $TMPDIR/addon/AnyKernel3/ramdisk /system/addon.d/$MODID-unityakfiles
